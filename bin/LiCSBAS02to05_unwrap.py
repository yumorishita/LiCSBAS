...
Usage
=====
LiCSBAS02to05_unwrap.py -i WORKdir [-M nlook] [--freq float] [--n_para int] [**unwrapping_parameters - see below]
 ...
 -M  Number of multilooking factor (Default: 1, no multilooking)
...
def main(argv=None):
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "hi:o:M:", ["help", "freq=", "n_para="])
    ...
    os.chdir(workdir)
    unw.process_frame('"$frame"', dolocal = True, ml="$multi $extraparam")
